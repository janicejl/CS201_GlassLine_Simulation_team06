	...
	public void checkGlassDetected()
	{
		...
		if (pressed == false)
		{
			...
			for (int k = 0; k < this.activePieces.size(); k++)
			{
				...
				//
				if (activePieces.get(k).getCenterX()>=getCenterX()-5&&activePieces.get(k).getCenterX()<=getCenterX()+5&&activePieces.get(k).getCenterY()>=getCenterY()-5&&activePieces.get(k).getCenterY()<=getCenterY()+5)
				{
					setIcon(sensorOn);
		...
		else
		{
			...
			// to red(not pressed).
			if (currentGlassPiece.getCenterX()<=getCenterX()-10||currentGlassPiece.getCenterX()>=getCenterX()+25||currentGlassPiece.getCenterY()<=getCenterY()-25||currentGlassPiece.getCenterY()>=getCenterY()+25)
			{
